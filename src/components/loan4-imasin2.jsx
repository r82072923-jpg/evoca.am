...
const Loan4iMasin2 = ({ activeTab, setActiveTab }) => {
    ...
    useEffect(() => {
        const fetchFromFirebase = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, "loans4iMasin"));
                if (!querySnapshot.empty) {
                    setLoanData(querySnapshot.docs[0].data());
                } else {
                    console.warn("Collection-ը դատարկ է:");
                }
            } catch (error) {
    ...
    if (loading) {
        return <div className="text-center py-10 text-gray-500 font-medium">Բեռնվում է տեղեկատվությունը...</div>;
    }

    if (!loanData) {
        return (
            <div className="w-full max-w-7xl mx-auto my-10 p-4 font-sans text-center">
                <p className="text-red-500 font-bold mb-4">Firebase-ում տվյալներ չեն գտնվել։</p>
            </div>
        );
    }
                ...
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
                    ...
                    <div className="flex-1 text-sm sm:text-base text-gray-800 leading-relaxed">
                        <p className="mb-6 font-medium">
                            ...
                            <span className="text-[#6b11cb] font-bold uppercase">
                                {loanData.leftSection?.introParagraph?.highlightText}
                            </span>
                        </p>

                        <ul className="space-y-4 mb-8 font-medium">
                            {loanData.leftSection?.listItems?.map((item, idx) => (
                                <li key={idx} className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-[#6b11cb] shrink-0"></div>
                                    <span>{item}</span>
                                </li>
                            ))}
                        </ul>
                        ...
                        <p>
                            {loanData.leftSection?.endParagraph?.startText}
                            <span className="text-[#6b11cb] font-bold">
                                {loanData.leftSection?.endParagraph?.highlightText}
                            </span>
                            {loanData.leftSection?.endParagraph?.endText}
                        </p>
                    </div>
