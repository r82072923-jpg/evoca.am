...
const DigitaliMasin2 = () => {
  ...
  const fetchDataFromFirebase = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        console.log("Փաստաթուղթը գոյություն չունի բազայում:");
      }
    } catch (error) {
      console.error("Սխալ տվյալներ կարդալիս:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!data) {
    return (
      <div className="flex justify-center items-center w-full min-h-[400px] bg-white">
        <p className="text-red-600 font-medium">Տվյալները չհաջողվեց գտնել բազայում:</p>
      </div>
    );
  }
  return (
      ...
      <div className="max-w-5xl w-full px-6 font-sans text-gray-800 space-y-8">
        <div className="space-y-6">
          <p className="text-sm md:text-base leading-relaxed">
            ...
            <span className="text-purple-700 font-medium">{data.introParagraphs?.[0]?.highlight}</span>, սերտիֆիկացումից և հետագա ծրագրային ապահովումից:
          </p>
          <p className="text-sm md:text-base leading-relaxed">
            {data.introParagraphs?.[1]?.text}
          </p>
          <p className="text-sm md:text-base leading-relaxed font-medium">
            {data.introParagraphs?.[2]?.text}
          </p>
          ...
          <ul className="space-y-3">
            <li className="flex items-start">
              ...
              <span className="text-sm md:text-base">
                <span className="font-bold text-purple-900">{data.benefits?.[0]}</span>
              </span>
            </li>
            <li className="flex items-start">
              <span className="inline-block w-2 h-2 rounded-full bg-purple-700 mt-2 mr-3 flex-shrink-0"></span>
