...
const DigitaliMasin2 = () => {
  ...
  const uploadDataToFirebase = async () => {
    try {
      setUploadStatus('Ուղարկվում է...');
      await setDoc(doc(db, 'digitaliMasin', 'content'), contentData);
      setUploadStatus('Տվյալները հաջողությամբ ուղարկվեցին բազա!');
      fetchDataFromFirebase();
    } catch (error) {
      console.error("Սխալ ուղարկելիս:", error);
      setUploadStatus('Սխալ տվյալների ուղարկման ժամանակ:');
    }
  };
  ...
  const fetchDataFromFirebase = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        console.log("Փաստաթուղթը գոյություն չունի, կարող եք ուղարկել այն:");
      }
    } catch (error) {
      console.error("Սխալ կարդալիս:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center w-full min-h-[400px] bg-white">
        <p className="text-purple-800 font-medium animate-pulse">Բեռնվում է բազայից...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col items-center w-full bg-white py-8">
      <div className="mb-6 p-4 bg-purple-50 rounded-lg border border-purple-200 flex flex-col items-center space-y-2">
        <button 
          onClick={uploadDataToFirebase}
          className="px-4 py-2 bg-purple-700 text-white font-medium rounded-md hover:bg-purple-800 transition-colors"
        >
          Ուղարկել տվյալները Firebase (`digitaliMasin`)
        </button>
          ...
          <div className="space-y-6">
              ...
              <table className="w-full text-left border-collapse border border-purple-100 text-sm">
                <thead>
                  <tr className="bg-purple-50/50 text-gray-700">
                    <th className="border border-purple-100 p-3 font-semibold w-1/4">{data.tableHeaders?.[0]}</th>
                    <th className="border border-purple-100 p-3 font-semibold">{data.tableHeaders?.[1]}</th>
                    <th className="border border-purple-100 p-3 font-semibold">{data.tableHeaders?.[2]}</th>
                    <th className="border border-purple-100 p-3 font-semibold">{data.tableHeaders?.[3]}</th>
                    <th className="border border-purple-100 p-3 font-semibold">{data.tableHeaders?.[4]}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-purple-100 p-3 font-medium bg-gray-50/50">{data.tableRows?.[0]?.type}</td>
                    <td className="border border-purple-100 p-3">{data.tableRows?.[0]?.arca}</td>
                    <td className="border border-purple-100 p-3">{data.tableRows?.[0]?.local}</td>
                    <td className="border border-purple-100 p-3">{data.tableRows?.[0]?.international}</td>
                    <td className="border border-purple-100 p-3">{data.tableRows?.[0]?.qr}</td>
                  </tr>
                  <tr>
                    <td className="border border-purple-100 p-3 font-medium bg-gray-50/50">{data.tableRows?.[1]?.type}</td>
                    <td className="border border-purple-100 p-3 font-bold" colSpan="4">{data.tableRows?.[1]?.value}</td>
                  </tr>
                  <tr>
                    <td className="border border-purple-100 p-3 font-medium bg-gray-50/50">{data.tableRows?.[2]?.type}</td>
                    <td className="border border-purple-100 p-3 font-bold" colSpan="4">{data.tableRows?.[2]?.value}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-start space-x-3">
                ...
                <p className="text-sm md:text-base text-gray-700 leading-relaxed">{data.steps?.[0]}</p>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <p className="text-sm md:text-base text-gray-700 leading-relaxed">{data.steps?.[1]}</p>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4">
              <h2 className="text-lg md:text-xl font-bold text-purple-900">
                {data.loans?.businessLoan?.title}
              </h2>
              <p className="text-sm md:text-base text-gray-700 leading-relaxed">
                {data.loans?.businessLoan?.description}
              </p>
              <h3 className="font-bold text-gray-900 pt-2">Վարկի համար կարող են դիմել՝</h3>
              <ul className="space-y-2">
                {data.loans?.businessLoan?.applicants?.map((item, index) => (
              ...
              <ul className="space-y-2">
                {data.loans?.businessLoan?.collateral?.map((item, index) => (
                  <li key={index} className="flex items-start">
                    <span className="inline-block w-2 h-2 rounded-full bg-purple-700 mt-2 mr-3 flex-shrink-0"></span>
                    <span className="text-sm md:text-base text-gray-700">{item}</span>
                  </li>
            ...
            <div className="space-y-4">
              <h2 className="text-lg md:text-xl font-bold text-purple-900">
                {data.loans?.businessOverdraft?.title}
              </h2>
              <p className="text-sm md:text-base text-gray-700 leading-relaxed">
                {data.loans?.businessOverdraft?.description}
              </p>
              <h3 className="font-bold text-gray-900 pt-2">Վարկի համար կարող են դիմել՝</h3>
              <ul className="space-y-2">
                {data.loans?.businessOverdraft?.applicants?.map((item, index) => (
                  <li key={index} className="flex items-start">
                    <span className="inline-block w-2 h-2 rounded-full bg-purple-700 mt-2 mr-3 flex-shrink-0"></span>
                    <span className="text-sm md:text-base text-gray-700">{item}</span>
                  </li>
