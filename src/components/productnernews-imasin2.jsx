...
const ProductnerNewsiMasin2 = () => {
  ...
  useEffect(() => {
    const fetchNewsData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "productnerinewsiMasin"));
        const data = [];
  ...
  for (let i = 0; i < newsData.length; i += itemsPerPage) {
    pages.push(newsData.slice(i, i + itemsPerPage));
  }
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-4 py-8 font-sans">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 min-h-[360px]">
        {pages[currentIndex]?.map((productsNews) => (
          <div key={productsNews.id} className="bg-white flex flex-col justify-between">
            <div>
              <div className="w-full h-[180px] overflow-hidden mb-3">
                <img
                  src={productsNews.imageSrc}
                  alt={productsNews.title}
                  className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
                />
              </div>

              <Link to={productsNews.link || "/news/banking"} className="group block mb-2">
...
export default ProductnerNewsiMasin2;