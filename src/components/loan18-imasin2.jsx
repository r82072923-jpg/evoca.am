...
const Loan18iMasin2 = ({ activeTab, setActiveTab }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "loans18iMasin"));
        if (!querySnapshot.empty) {
          const firstDoc = querySnapshot.docs[0].data();
          setLoanData(firstDoc);
        }
      } catch (error) {
  ...
  const tabs = loanData.tabs || ['Վարկի մասին', 'Պայմաններ', 'Պահանջվող փաստաթղթերի ցանկ'];
