...
const Slayder4 = () => {
  ...
  useEffect(() => {
    const fetchLoansFromFirebase = async () => {
      try {
        ...
        const querySnapshot = await getDocs(collection(db, 'slayder4'));

        if (!querySnapshot.empty) {
          const docData = querySnapshot.docs[0].data();
          if (docData.loans && Array.isArray(docData.loans)) {
            setLoans(docData.loans);
          }
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="w-full py-16 text-center font-sans text-[#6b11cb] font-semibold">
        Բեռնվում է...
      </div>
    );
  }
        ...
        <div className="flex flex-1 justify-center gap-4 md:gap-6 overflow-hidden">
          {visibleIndices.map((index) => {
            ...
            return (
              ...
              >
                <Link to={loan.link}>
                  <div className={`w-full h-40 ${loan.bgColor || ''} rounded-xl mb-4 flex items-center justify-center overflow-hidden transition-transform duration-300 group-hover/card:scale-[1.02]`}>
