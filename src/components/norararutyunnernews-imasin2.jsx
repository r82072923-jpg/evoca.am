...
const NorararutyunnerNewsiMasin = () => {
  ...
  useEffect(() => {
    const fetchNewsData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "norararutyunneriMasin"));
        const data = [];
        querySnapshot.forEach((doc) => {
          data.push({ id: doc.id, ...doc.data() });
        });

        // Տվյալները դասավորում ենք ըստ ID-ի (1, 2, 3...)
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-4 py-8 font-sans">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 min-h-[360px]">
        {pages[currentIndex]?.map((item) => (
          <div key={item.id} className="bg-white flex flex-col justify-between">
            <div>
              ...
              <Link to={item.link || "/news/innovations"} className="group block mb-2">
