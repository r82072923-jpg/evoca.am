...
function TopHeader2() {
  ...
  const fetchHeaderData = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "topHeader2"));
      const items = [];
  ...
  // Փակել բջջային մենյուն էջ փոխելիս
    ...
    <header className="w-full bg-white shadow-sm border-b border-gray-100 z-50 relative">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 flex items-center justify-between h-[72px]">
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-0 w-56 bg-white shadow-xl rounded-b-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top group-hover:translate-y-0 translate-y-2 border border-gray-50">
                    <div className="py-2 flex flex-col">
                      {item.subItems.map((subItem, index) => {
                        ...
                        const linkClasses = "px-5 py-3 text-[14px] font-medium text-slate-700 hover:text-[#6b21a8] hover:bg-purple-50 transition-colors block";
        ...
        <div className="flex items-center gap-3 md:gap-4 z-50">
          <Link
            to="/evoca-online"
            className="hidden sm:flex bg-[#5b21b6] hover:bg-[#4c1d95] text-white font-bold text-[14px] px-6 py-2.5 rounded-full transition-all items-center justify-center tracking-wide"
          >
            EvocaONLINE
          </Link>
      ...
      <div 
        className={`lg:hidden absolute top-full left-0 w-full bg-white shadow-lg border-t border-gray-100 transition-all duration-300 overflow-hidden ${
      ...
      >
        <div className="px-4 py-4 flex flex-col gap-2">
            ...
            navItems.map((item) => (
              <div key={item.id || item.path} className="flex flex-col border-b border-gray-50 last:border-0 pb-1">
                {item.subItems && item.subItems.length > 0 ? (
                  ...
                  >
                    {item.title}
                    <i className={`fa-solid fa-chevron-down text-[12px] transition-transform duration-300 ${activeMobileSubMenu === (item.id || item.path) ? 'rotate-180' : ''}`}></i>
                  </button>
                ) : (
                  <NavLink
                    to={item.path}
                    ...
                    className={({ isActive }) =>
                      `py-3 px-2 text-[15px] font-bold ${isActive ? "text-[#6b21a8]" : "text-slate-800"}`
                    }
                  >
                    {item.title}
                  </NavLink>
                ...
                {item.subItems && item.subItems.length > 0 && (
                  <div className={`flex flex-col ml-4 space-y-1 overflow-hidden transition-all duration-300 ${activeMobileSubMenu === (item.id || item.path) ? 'max-h-[400px] py-2' : 'max-h-0'}`}>
                    {item.subItems.map((subItem, index) => {
                      const isPhoneLink = subItem.path.startsWith("tel:");
          ...
          <div className="sm:hidden mt-4 mb-2 px-2">
            <Link
              to="/evoca-online"
              onClick={closeMobileMenu}
              className="block w-full text-center bg-[#5b21b6] text-white font-bold text-[15px] py-3 rounded-full shadow-md active:scale-95 transition-transform"
            >
              EvocaONLINE
            </Link>
          </div>
