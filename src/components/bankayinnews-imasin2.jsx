...
const BankayinNewsiMasin2 = () => {
  const [bankingNewsData, setBankingNewsData] = useState([]);
  ...
  useEffect(() => {
    const fetchNewsData = async () => {
      try {
        const newsCollectionRef = collection(db, 'bankayinnewsiMasin');
        const data = await getDocs(newsCollectionRef);
        
        const fetchedNews = data.docs.map((doc) => ({
          ...doc.data(),
          firebaseId: doc.id,
        }));

        fetchedNews.sort((a, b) => a.id - b.id);

        setBankingNewsData(fetchedNews);
        setIsLoading(false);
      } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-[360px] flex items-center justify-center">
        <p className="text-xl font-semibold text-[#5b00c9]">Տվյալները բեռնվում են...</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 py-8 font-sans">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 min-h-[360px]">
        {pages[currentIndex]?.map((bankingNews) => (
          <div key={bankingNews.firebaseId || bankingNews.id} className="bg-white flex flex-col justify-between">
            <div>
...
export default BankayinNewsiMasin2;